...
SYMBOL_DISPLAY = "BTC-USD (M5)"
TIMEZONE_LOCAL = ZoneInfo("America/Panama")

TEMPORALIDAD_MINUTOS = 5  # Configurado para velas M5
HORAS_PROYECCION = 2  # Bloque de proyectado de 2 horas
...
class HealthCheckHandler(BaseHTTPRequestHandler):
  ...
  def do_GET(self):
    ...
    self.end_headers()
    self.wfile.write(b"Bot activo catalogando M5")
...
# ===== DATOS DE MERCADO EN VELAS M5 =====
def get_market_data():
  # Solicitamos velas de 5 minutos (interval=5m)
  url = f"https://query1.finance.yahoo.com/v8/finance/chart/{SYMBOL_YAHOO}?range=5d&interval=5m"
  try:
    ...
    print(f"Error en get_market_data M5: {e}", flush=True)
    return pd.DataFrame()


# ===== PROYECCIÓN M5 PARA PUNTOS ALTOS Y BAJOS =====
def proyectar_horarios():
  ...
  # Espaciado de 10 minutos entre señales en M5
  intervalo_minutos = 10

  for m in range(TEMPORALIDAD_MINUTOS, total_minutos, intervalo_minutos):
    idx = -(m % 20) - 1
    ...
    # PUNTO ALTO EN M5 = VENDER (ABAJO)
    es_punto_alto = (
        (rsi_val >= 60) or (stoch_val >= 70) or (close_val >= bb_h_val)
    )

    # PUNTO BAJO EN M5 = COMPRAR (ARRIBA)
    es_punto_bajo = (
    ...
    # Entrada a salida toma exactamente 5 minutos
    hora_salida = hora_entrada + timedelta(minutes=TEMPORALIDAD_MINUTOS)
...
def construir_mensaje_panel(entrada_activa=None):
  ...
  if entrada_activa:
    ...
    texto += (
        ...
        f" {entrada_activa['hora_salida']} (M5)"
    )

  return texto


# ===== EVALUADOR Y PROCESADOR DE ENTRADAS/SALIDAS =====
def procesar_catalogador():
  ...
  for s in LISTA_SENALES:
    # Captura de precio al iniciar el minuto de entrada
    if (
        s["estado"] == "PENDIENTE"
      ...
      print(
          f"[{hora_actual_str}] Entrada M5 registrada en {precio_actual} para"
          f" {s['direccion']}",
          flush=True,
      )
    ...
    # Evaluación al cumplirse los 5 minutos (hora_salida)
    if (
        s["estado"] == "PENDIENTE"
        and s["precio_entrada"] is not None
      ...
      print(
          f"[{hora_actual_str}] Operación M5 Finalizada: {s['estado']} |"
          f" Entrada: {s['precio_entrada']} -> Salida: {s['precio_salida']}",
          flush=True,
      )
      actualizar_panel = True
...
if __name__ == "__main__":
  ...
  if LISTA_SENALES:
    ...
    print(
        f"--- PANEL M5 PUBLICADO EXITOSAMENTE (ID: {PANEL_MESSAGE_ID}) ---",
        flush=True,
    )
